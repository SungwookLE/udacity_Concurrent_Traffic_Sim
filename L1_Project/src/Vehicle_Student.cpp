...
{
    // (8/1) DONE: argument with class(*this) and class member(drive()) as  & reference
    ...
    std::thread tt(&Vehicle::drive, this);
    _threads.emplace_back(std::move(tt));

    // or : _threads.emplace_back(std::thread(&Vehicle::drive,this));
